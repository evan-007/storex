defmodule Storex.Sales do
  ...
  def line_items_quantity_count(line_items) do
    Enum.reduce(line_items, 0, fn(line_item, count) ->
      count + line_item.quantity
    end)
  end

  def line_items_total_price(line_items) do
    Enum.reduce(line_items, Decimal.new(0), fn(line_item, total) ->
      quantity = Decimal.new(line_item.quantity)
      partial = Decimal.mult(quantity, line_item.book.price)

      Decimal.add(total, partial)
    end)
  end
end
